...
## Display the images
sp_img = sp_noise(img_GRAY, 0.1)
 
cv2.namedWindow("SP Noise Image", cv2.WINDOW_NORMAL)
cv2.imshow("SP Noise Image", sp_img)
cv2.imwrite("./OutputData/SP-Image.jpg", sp_img)
cv2.waitKey()

possion_img = possion_noise(img_GRAY)
cv2.namedWindow("Possion Noise Image", cv2.WINDOW_NORMAL)
cv2.imwrite("./OutputData/Possion_Img.jpg", possion_img)
cv2.imshow("Possion Noise Image", possion_img)
cv2.waitKey()
...
t = time.time()
blur = cv2.filter2D(sp_img, -1, kernel) # changed this 
...
t = time.time()
blur = cv2.GaussianBlur(sp_img, (5,5), 0)
tmpRunTime = time.time() - t
...
t = time.time()
blur = cv2.bilateralFilter(sp_img, 9, 5, 5)
tmpRunTime = time.time() - t
